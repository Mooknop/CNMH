 ...
 *  5. SHIELD-BLOCK RIDER (#1035/#1040, reminder half): a Retaliation-runed
 *     shield states its onBlock rider while raised, and a block folds the
 *     rider summary into the encounter log. KNOWN GAP, deliberately not
 *     asserted: the interactive follow-up (pick attacker → enter rolled dice →
 *     save request to the GM) is currently UNREACHABLE live — #1341 made
 *     every block call lowerShield(), which unmounts ShieldBlockBar (guard:
 *     `if (!heldShield || !raised) return null`) on the same render that arms
 *     the follow-up. The unit tests mock useShield with a constant
 *     `raised: true`, which is why they pass. Extend this test to drive the
...
test.describe('Accessory runes', () => {
  ...
  test('storefront etch flow: stage → checkout hands the host over → collect returns it inscribed', async ({
    ...
    const etchOption = picker.getByRole('button', { name: `etch ${MENACING.name}` });
    // The offered price is the RUNESTONE ware's price — rune + blank etching
    // stone (utils/runestone.js RUNESTONE_BASE, 3 gp) — so read the number the
    // shop itself asks rather than restating the composition here.
    const etchPrice = parseInt(await etchOption.locator('.ps-runeopt-price').innerText(), 10);
    expect(etchPrice).toBeGreaterThanOrEqual(MENACING.price);
    await etchOption.click();

    // Staged, not paid: the pending note names the price and the 24h handoff.
    await expect(page.getByTestId(`staged-${UID_FINE}`)).toContainText(`${etchPrice} gp`);
    ...
    await session.expectSent(`cnmh_gold_${CHAR_ID}`, (v) => v === 100 - etchPrice);
  ...
  test('retaliation rider: the raised runed shield states its rider, and a block logs it', async ({
    page,
    seed,
  }) => {
    ...
    // NOT asserted — the armed follow-up (attacker picker → rolled 4d4 → save
    // request). It is unreachable live: #1341's lowerShield-on-block unmounts
    // the bar (raised → false) on the same render that arms it, so the local
    // `armed` state is lost. When that is fixed, extend this test: after the
    // block, select the goblin in `shieldblock-rune-followup`, enter the
    // rolled damage, click "use Retaliation (Lesser)", and assert
    // cnmh_encounter_global.saveRequests gains { abilityName, save: 'reflex',
    // dc: 20, basic: true } plus the `${uid}:actuated` cnmh_freq_ record.
  });
});
