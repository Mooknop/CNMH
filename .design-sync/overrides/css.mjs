// forked from design-sync lib/css.mjs — copyTokens: repo-local token stylesheets
// (this is an app repo; tokens live in src/*.css, not a node_modules package).
...
export function copyTokens({ tokensPkg, tokensGlob, nodeModules, out }) {
  const tokenFiles = [];
  // FORK: no tokens package + tokensGlob set → entries are workspace-relative
  // stylesheet paths (string or array). Each is copied into tokens/ with its
  // @font-face url()s repointed at fonts/ copies (tokens/ sits one level below
  // the bundle root), so the shipped face never dangles or shadows a working one.
  if (!tokensPkg && tokensGlob) {
    const workspaceRoot = dirname(resolve(nodeModules));
    const contained = (p) => {
      const rel = relative(workspaceRoot, p);
      return rel !== '' && !rel.startsWith('..') && !isAbsolute(rel);
    };
    for (const relPath of Array.isArray(tokensGlob) ? tokensGlob : [tokensGlob]) {
      const srcFile = resolve(workspaceRoot, relPath);
      if (!existsSync(srcFile) || !contained(srcFile)) {
        console.error(`  ! tokensGlob: ${relPath} not found under the workspace — skipped`);
        continue;
      }
      let css = readFileSync(srcFile, 'utf8');
      css = css.replace(/url\(\s*['"]?([^'")]+?\.(?:woff2?|ttf|otf))['"]?\s*\)/gi, (whole, u) => {
        if (/^(https?:|data:)/.test(u)) return whole;
        const fontFile = resolve(dirname(srcFile), u);
        if (!existsSync(fontFile) || !contained(fontFile)) return whole;
        mkdirSync(join(out, 'fonts'), { recursive: true });
        cpSync(fontFile, join(out, 'fonts', basename(fontFile)));
        return `url('../fonts/${basename(fontFile)}')`;
      });
      mkdirSync(join(out, 'tokens'), { recursive: true });
      writeFileSync(join(out, 'tokens', basename(srcFile)), css);
      tokenFiles.push(basename(srcFile));
    }
    if (tokenFiles.length) console.error(`  tokens: ${tokenFiles.length} repo-local file(s) via tokensGlob (forked css.mjs)`);
    return tokenFiles;
  }
