...
| `doorreq` | app → bridge | `global` | `{ ts }` — **protocol ≥ 20 (`SCENE_DOORS_PROTOCOL`, #1805):** request EVERY door on the rendered scene for the GM dock's door overlay. No adjacency gate |
| `dooropts` | bridge → app | `global` | `{ doors: [{ wallId, state, x, y, secret? }], sceneId, reqTs }` — protocol ≥ 20. Every door on the scene, whatever its state or distance. **Secret doors (wall `door` type 2) ARE included**, each carrying `secret: true` (the field is absent for regular doors) so the GM map can draw them with a distinct glyph — the per-character feed above keeps its hidden-unless-open rule, so players never see an unrevealed secret door. Wire-privacy caveat (the relay fans out to all clients) accepted by user ruling on #1805. `sceneId` is the scene the coordinates belong to; `reqTs` echoes the request's `ts`, or is `null` for an unprompted re-push. The bridge **re-pushes this key from Foundry's `updateWall` hook on every door-state (`ds`) change — including app-initiated ones** — so the overlay tracks doors opened natively in Foundry or by another player |
| `doorinteract` | app → bridge | `global` | `{ wallId, op, ts }` — protocol ≥ 20. Identical handling to the per-character form: the `wallId` alone identifies the door, so one handler serves both id spaces |
| `exploremove` | both | `global` | `boolean` — exploration-movement toggle. App-owned (`usePlayMode`); the bridge force-writes `false` when any door opens (auto-off). The auto-off latch is the ONLY thing the bridge's own `_bridgeSource` echo skip suppresses — the `dooropts_global` re-push above fires for app-driven door changes too |
| `playmode` | app → bridge | `global` | `'exploration'\|'downtime'` — the GM-set non-combat mode (#1807, protocol ≥ 21). App-owned (`usePlayMode`'s `gmMode`); the bridge only READS it (tracked from FULL_STATE + every `UPDATE`, default `'exploration'` matching the app's own) so the snapshot rail's post-`movedone` broadcast knows whether to frame the mover or the whole party — see `snapdone` below. An active Foundry combat overrides the tracked value the same way `usePlayMode`'s `encounter.active` check does |
| `research` | app-only (not relayed) | `global` | `{ [topicId]: { available, rp, perSourceRp: { [sourceName]: number } } }` — the GM Command Dock's Downtime pane (#1841/#1843, epic #206): party progress against `research` collection topic docs (capture-only, GM-authored — see `GmResearch`, #1842). `available` opens a topic to the party (anti-metagaming gate — a topic the GM hasn't flipped renders collapsed, no sources/tiers visible); `rp`/`perSourceRp` are the accrued research points, each source capped at its own `maxRp` (`src/utils/research.js` owns all the math). Listed here for completeness only — the bridge never reads or writes this key, and there is no Foundry-side mirror to keep in sync (contrast `playmode` above, which the bridge tracks) |
...
| `snapreq` | app → bridge | `global` | `{ id, ts, moverId?, radiusFeet?, party? }` — request a scene snapshot (#1573 B1: Ping the Map / template placement). Without `moverId`/`party` this captures the GM client's current canvas view, unchanged. With `moverId` (protocol ≥ 16, #1744 WS-2) it captures the WORLD RECT centred on that mover instead — `radiusFeet` in every direction, clamped to the canvas bounds, defaulting to **1.5× the mover's Speed** — so a player's destination is in frame regardless of where the GM is looking. `moverId` resolves through the same id spaces as every movement key (PC charId, minion `<ownerCharId>-<role>`, combat entryId); one that doesn't resolve falls back to the GM view rather than nacking. With `party: true` (protocol ≥ 21, #1807) it instead captures the WORLD RECT framing every actor-mapped PC token on the rendered scene, plus a margin — see `snapdone`'s `tokens` field below for the framing rule. No party member on the rendered scene falls back to the GM view too. Live-only: never replayed from FULL_STATE |
| `snapdone` | bridge → app | `global` | `{ id, ok, url?, capture?, worldRect?, gridSize?, tokens?, moverId, trigger, ts }` — ack for `snapreq` (`id` echoes). `url` = stable app-relative `/api/images/…` (the bestiary-token R2 pipeline, catalog folder `Scene Snapshots`; captures are never content-referenced, so the orphan sweep is their GC). `capture` = the world→capture-space affine matrix `{ a, b, c, d, tx, ty, screenW, screenH, sceneId }` — the app inverts it to map a normalized tap back to world coordinates; `worldRect` = `{ x1, y1, x2, y2 }` the rect the image covers, in world coords (matrix-less fallback); `gridSize` = px per square. **Those three describe the captured rect whatever produced it** — a mover-centered OR party-framed capture reports its own rect with the same semantics, so `worldPointFromTap` / `cellFromWorldPoint` invert it unchanged. `moverId` = the mover a mover-centered capture is framed on, `null` for a party-framed OR GM-view capture; `trigger` = `'request'` (answering a `snapreq`) or `'movedone'` (protocol ≥ 16 — the ONE broadcast capture the bridge pushes unprompted after every completed move, so N viewing clients cost one capture instead of N; correlate by `id` as always, a broadcast never matches a pending request). **`tokens` (protocol ≥ 21, #1807) rides ONLY on a party-framed capture** — `[{ moverId, x, y }]`, the world-space CENTRE of each party token that made it into frame (top-left + half its footprint, same geometry `moverCaptureRect` already frames around) — so the app can draw tappable PC targets over the image; absent on a mover-centered or GM-view ack. The party box is the bounding box of every in-frame token's own `moverCaptureRect` at a shared margin — **1.5× the fastest in-frame party member's Speed** (falling back to the plain 30 ft radius), the same multiplier a single mover's default radius uses — which IS the bbox expanded by that margin, since each per-token rect is already clamped to canvas bounds. **While the GM-set play mode (`playmode`) is `'exploration'` and no Foundry combat is active, the post-`movedone` broadcast captures the PARTY rect instead of the single mover's** — every client's party map refreshes after each move; encounter and downtime broadcasts stay mover-centered. GM-only layers (notes, drawings, HUD, rulers) and hidden tiles/tokens are excluded from the image; measured templates stay visible. `ok:false` = no snapshot available — fall back now |
...
- **Relay contract tests** (`relayContract.test.js`, #1308) — the same tripwire
  ...
  and fix the failing app consumers. A channel whose `global` id form carries a
  DIFFERENT payload than its per-character form gets its own recipe and its own
  fixture file — `dooropts` (per-char) and `dooropts_global` (#1805) are the
