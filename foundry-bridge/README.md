...
| `snapreq` | app → bridge | `global` | `{ id, ts }` — request a scene snapshot of the GM client's current canvas view (#1573 B1: Ping the Map / template placement). Live-only: never replayed from FULL_STATE |
| `snapdone` | bridge → app | `global` | `{ id, ok, url?, capture?, worldRect?, gridSize?, ts }` — ack for `snapreq` (`id` echoes). `url` = stable app-relative `/api/images/…` (the bestiary-token R2 pipeline, catalog folder `Scene Snapshots`; captures are never content-referenced, so the orphan sweep is their GC). `capture` = the stage worldTransform at capture time `{ a, b, c, d, tx, ty, screenW, screenH, sceneId }` — the app inverts it to map a normalized tap back to world coordinates; `worldRect` = `{ x1, y1, x2, y2 }` viewport in world coords (matrix-less fallback); `gridSize` = px per square. GM-only layers (notes, drawings, HUD, rulers) and hidden tiles/tokens are excluded from the image; measured templates stay visible. `ok:false` = no snapshot available — fall back now |
...
| `templateplace` | app → bridge | `global` | `{ id, shape, feet, x, y, sceneId, name?, ts }` — draw a measured template for a placed area (#1573 B4). WORLD coordinates (the app resolved them); radial shapes only (`burst`/`emanation` → a circle) because a cone/line needs a facing the app can't infer. The bridge also pings the centre. Scene-guarded |
| `templatedone` | bridge → app | `global` | `{ id, ok, templateId?, ts }` — ack for `templateplace` (`id` echoes). `templateId` is the created MeasuredTemplate's id, echoed so a later cleanup rail can remove it; `ok:false` = no outline was drawn (wrong scene, non-radial shape, or the create failed) and the app says so in the log |
