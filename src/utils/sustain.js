...
// once a later turn passes without a sustain.
export const makeSustainEntry = ({ ability, round }) => ({
  id:                 newEntryUid(),
 ...
 *
 * @param {Object}   ability    - the cast spell { name, duration }
 ...
 */
export function registerSustain({ ability, caster, round, getState, sendUpdate, appendLog }) {
  if (!isSustainedSpell(ability) || !caster?.id) return;
  const current = getState(caster.id, 'sustains') || [];
  const entry = makeSustainEntry({ ability, round });
