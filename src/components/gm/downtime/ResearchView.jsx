...
// Downtime dock — Research view (#1841, epic #206 S3; moved out of
// DockDowntimePane into its own view file by the #1853 redesign). The party's
// research board: one card per GMG Research Topic (`researchTopics`, the
// capture-only `research` collection) over the single shared progress key
...
//
// WAVE-1 SCROLL EXCEPTION: the body still carries the pre-redesign card list
// and `.dock-dt-view-body--scroll`. The no-scroll re-layout (topic list column
// + detail pane) is #1854; the shell around this view is already no-scroll
// correct, so that slice replaces the body and nothing else.
...
const ResearchView = () => {
  ...
  const openCount = topics.filter((t) => topicProgress(progress, t.id).available).length;
      ...
      <header className="dock-dt-head">
        ...
        {topics.length > 0 && (
          <span className="dock-dt-count">
            {openCount} open · {topics.length - openCount} not yet open
          </span>
        )}
