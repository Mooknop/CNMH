...
jest.mock('../../utils/treatWounds', () => ({
  ...jest.requireActual('../../utils/treatWounds'),
  applyTreatWounds: jest.fn(),
}));
...
describe('DC buttons', () => {
  ...
  it('shows "training required" notice when medicine rank is 0', () => {
    ...
    const original = useCharacter;
    jest.spyOn(require('../../hooks/useCharacter'), 'useCharacter').mockReturnValue({
...
describe('immunity block', () => {
  it('shows immunity notice and disables confirm when target is immune', () => {
    ...
    const { IMMUNITY_EFFECT_ID } = jest.requireActual('../../utils/treatWounds');
...
describe('confirm handling', () => {
  it('calls applyTreatWounds and onClose on confirm (success)', () => {
    const { applyTreatWounds } = require('../../utils/treatWounds');
    ...
    expect(applyTreatWounds).toHaveBeenCalledWith(expect.objectContaining({
