...
jest.mock('../shared/ActionIcon', () =>
  function DummyActionIcon({ actionText }) {
    return <span data-testid="action-icon">{actionText}</span>;
  }
);

jest.mock('../shared/UseActionChip', () =>
  function DummyUseActionChip({ name, cost, onUse, inactive }) {
    return (
      <button
...
describe('ActionDetailModal', () => {
  it('renders nothing when isOpen is false', () => {
    const { container } = render(
      <ActionDetailModal item={baseItem} isOpen={false} onClose={jest.fn()} />
    );
  ...
  it('renders nothing when item is null', () => {
    const { container } = render(
      <ActionDetailModal item={null} isOpen onClose={jest.fn()} />
    );
    expect(container.firstChild).toBeNull();
  });

  it('renders the item name as modal title', () => {
    render(<ActionDetailModal item={baseItem} isOpen onClose={jest.fn()} />);
  ...
  it('renders Reaction action text for reaction type', () => {
    ...
    render(<ActionDetailModal item={reaction} type="reaction" isOpen onClose={jest.fn()} />);
  ...
  it('renders Free Action text for free-action type', () => {
    ...
    render(<ActionDetailModal item={fa} type="free-action" isOpen onClose={jest.fn()} />);
  ...
  it('does not render trigger box for non-reaction types', () => {
    ...
    render(<ActionDetailModal item={item} type="action" isOpen onClose={jest.fn()} />);
  ...
  describe('encounterMode', () => {
    it('shows UseActionChip in encounter mode for non-activity', () => {
      render(
        <ActionDetailModal item={baseItem} type="action" isOpen encounterMode onUse={jest.fn()} onClose={jest.fn()} />
      );
    ...
    it('does not show UseActionChip when encounterMode is false', () => {
      render(
        <ActionDetailModal item={baseItem} type="action" isOpen encounterMode={false} onUse={jest.fn()} onClose={jest.fn()} />
      );
    ...
    it('does not show UseActionChip for activity type', () => {
      const activity = { name: 'Scout', description: 'Scouting.', traits: [] };
      render(
        <ActionDetailModal item={activity} type="activity" isOpen encounterMode onUse={jest.fn()} onClose={jest.fn()} />
      );
      expect(screen.queryByTestId('use-action-chip')).not.toBeInTheDocument();
    });
  ...
  describe('activity type', () => {
    ...
    it('shows Set as active button for activity type', () => {
      ...
      render(
        <ActionDetailModal item={activity} type="activity" isOpen onClose={jest.fn()} onSetActive={onSetActive} isActive={false} />
      );
    ...
    it('shows Active — Clear label when isActive is true', () => {
      render(
        <ActionDetailModal item={activity} type="activity" isOpen onClose={jest.fn()} onSetActive={jest.fn()} isActive />
      );
    ...
    it('does not show Set as active button when onSetActive is not provided', () => {
      render(
        <ActionDetailModal item={activity} type="activity" isOpen onClose={jest.fn()} />
      );
