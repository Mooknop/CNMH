...
vi.mock('./ActionCardList', () => ({ default: ({ items, type }) => (
...
describe('ActionCategoryModal', () => {
  it('renders nothing visible when closed', () => {
    const { container } = render(
      <ActionCategoryModal isOpen={false} onClose={vi.fn()} title="Offensive" items={items} />
    );
  ...
  it('renders ActionCardList with items when open', () => {
    render(
      <ActionCategoryModal isOpen onClose={vi.fn()} title="Offensive" items={items} />
    );
  ...
  it('does not render StrikesList when showStrikes is false', () => {
    render(
      <ActionCategoryModal isOpen onClose={vi.fn()} title="Defensive" items={items} showStrikes={false} />
    );
