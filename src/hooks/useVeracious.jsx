...
export const useVeracious = (charId, inventory = []) => {
  ...
  // Imbued ring-rune names, for the "these effects apply to a modified attack"
  // reminder. Resolution may leave runes.property as ids or hydrated docs.
  const imbuedRunes = useMemo(() => {
    const prop = ring && ring.runes && Array.isArray(ring.runes.property) ? ring.runes.property : [];
    return prop.map((p) => (typeof p === 'string' ? p : p && (p.name || p.id))).filter(Boolean);
  }, [ring]);
  ...
  return { ring, itemBonus, imbuedRunes, armed, arm, disarm };
};
