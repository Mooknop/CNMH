...
jest.mock('../components/party/CharacterInventorySection', () => {
  return function DummyCharacterInventorySection({ character, onItemClick, items }) {
    return (
...
jest.mock('../components/inventory/ItemModal', () => {
  return function DummyItemModal({ isOpen, item, onClose }) {
...
jest.mock('../utils/CharacterUtils', () => ({
  getCharacterColor: jest.fn((i) => `#color${i}`),
}));

jest.mock('../utils/InventoryUtils', () => ({
  calculateItemsBulk: jest.fn(() => 1),
  formatBulk: jest.fn((b) => (b === 0 ? '—' : String(b))),
}));

jest.mock('../data/campaign', () => ({
  PARTY_GOLD: 1500,
}));
