...
jest.mock('../../components/gm/HistoryModal', () => () => null);
...
const setContent = (overrides = {}) =>
  useContent.mockReturnValue({ effects, syncEffects: jest.fn(), ...overrides });
...
describe('GmEffects', () => {
  ...
  it('renders empty state when no effects', () => {
    useContent.mockReturnValue({ effects: [], syncEffects: jest.fn() });
    render(<GmEffects />);
