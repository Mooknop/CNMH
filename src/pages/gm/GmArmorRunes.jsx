  ...
  return (
    <div className="gm-card" data-testid={`armor-rune-form-${e.id || 'new'}`}>
      ...
      <GmEntryDialogs
        ...
        name={e.name}
        isNew={isNew}
        deleteMessage={`Permanently delete the rune "${e.name}". This cannot be undone. (A standard seeded rune will reappear from the bootstrap catalog.)`}
        onRestored={(doc) => {
...
// armor rune edits with its own modifiers/riders form there too; the modifier
// vocabularies also seed the accessory rune form (#1033 S4).
export { RuneForm as ArmorRuneForm, toForm as armorToForm, blankRune as armorBlankRune, MODIFIER_STATS, MODIFIER_KINDS };
