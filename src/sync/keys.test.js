import { GLOBAL_ID, RELAY, syncKey, globalKey } from './keys';
...
describe('sync-key registry', () => {
  it('every relay type is a single lowercase token with no underscores', () => {
    for (const [name, type] of Object.entries(RELAY)) {
      expect(type, `RELAY.${name}`).toMatch(/^[a-z0-9]+$/);
    }
  });

  it('relay type values are unique', () => {
  ...
  it('parses round-trip for every relay type against the global id', () => {
